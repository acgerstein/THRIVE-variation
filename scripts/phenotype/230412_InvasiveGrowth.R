...
d <- read.csv(here("invasive_growth", "data_in", "230412_IG_YST6_YST7_TVY4_TVY10.csv"))
...
d$cols[d$Strain=="TVY4" & d$Place=="V"] <- "mediumpurple4"
d$cols[d$Strain=="TVY4" & d$Place=="R"] <- "mediumpurple"
d$cols[d$Strain=="TVY1" & d$Place=="V"] <- "violetred3"
d$cols[d$Strain=="TVY1" & d$Place=="R"] <- "palevioletred2"
d$cols[d$Strain=="YST7" & d$Place=="V"] <- "darkred"
d$cols[d$Strain=="YST7" & d$Place=="R"] <- "salmon"
d$cols[d$Strain=="YAG0" & d$Place=="0"] <- "black"

t <- jitter(as.numeric(as.factor(d$SP)))

pdf("figures/FigureX-IV.pdf", width=7, height=5.5, font = "Times")
...
wilcox.test(subset(d, SP == "YST6R")$max, subset(d, SP == "YST6V")$max ) #W = 252, p-value = 0.279
wilcox.test(subset(d, SP == "TVY4R")$max, subset(d, SP == "TVY4V")$max ) #W = 372, p-value = 0.03202 # R > V
wilcox.test(subset(d, SP == "TVY1R")$max, subset(d, SP == "TVY1V")$max ) #W = 372, p-value = 0.7447
wilcox.test(subset(d, SP == "YST7R")$max, subset(d, SP == "YST7V")$max ) #W = 136, p-value = 0.0001317 # V > R

wilcox.test(subset(d, SP == "YST6R")$mean, subset(d, SP == "YST6V")$mean ) #W = 252, p-value = 0.279
wilcox.test(subset(d, SP == "TVY4R")$mean, subset(d, SP == "TVY4V")$mean ) #W = 395.5, p-value = 0.01675
wilcox.test(subset(d, SP == "TVY1R")$mean, subset(d, SP == "TVY1V")$mean ) #WW = 292, p-value = 0.9308
wilcox.test(subset(d, SP == "YST7R")$mean, subset(d, SP == "YST7V")$mean ) #W = 115.5, p-value = 0.0001846
...
pdf("Figures_Tables/FigureSX-IV.pdf", width=6, height=7, font = "Times")
